...
class ZhiHuEncrypt:
    ...
    def G_f(tt: int):
        ...
        return (
            ti
            ^ ZhiHuEncrypt.Q_f(ti, 2)
            ^ ZhiHuEncrypt.Q_f(ti, 10)
            ^ ZhiHuEncrypt.Q_f(ti, 18)
            ^ ZhiHuEncrypt.Q_f(ti, 24)
        )
    ...
    def __g_r(tt: list) -> list:
        ...
        for i in range(0, 32):
            ta = ZhiHuEncrypt.G_f(
                tr[i + 1] ^ tr[i + 2] ^ tr[i + 3] ^ h_zk[i]
            )  # 这里的h_zk同样也是一个固定的32位数组
            tr[i + 4] = tr[i] ^ ta
    ...
    def final_encrypt(m_v):
        ...
        for i in range(len(m_v) - 1, -1, -3):
            ...
            if i % 4 == 3:
                a_v = (
                    (m_v[i] ^ 58) + (sls(m_v[i - 1], 8)) + (sls(m_v[i - 2], 16))
                )  # 此处M_v为基础逻辑中计算得出
            elif i % 4 == 0:
                a_v = m_v[i] + (sls(m_v[i - 1] ^ 58, 8)) + (sls(m_v[i - 2], 16))
            elif i % 4 == 1:
                a_v = (
                    m_v[i] + (sls(m_v[i - 1], 8)) + (sls(m_v[i - 2] ^ 58, 16))
                )  # 此处M_v为基础逻辑中计算得出
            else:
                a_v = (
                    m_v[i] + (sls(m_v[i - 1], 8)) + (sls(m_v[i - 2], 16))
                )  # 此处M_v为基础逻辑中计算得出
    ...
    def get_result(url: str):
        ...
        if not d_c0_cache:
            d_c0_cache = ZhiHuEncrypt.get_d_c0()
        d_c0 = d_c0_cache
...
async def clear_cache():
    global d_c0_cache
    d_c0_cache = None
    return {"message": "d_c0 cache cleared"}

@app.get("/d_c0/")
async def get_d_c0():
    global d_c0_cache
    return {"d_c0": d_c0_cache}
