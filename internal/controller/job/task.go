...
func (h *Controller) addTaskToCronService(taskID, cronExpr string, include, exclude []string, taskType int) (jobID string, err error) {
	...
	case cronDB.TypeZhihu:
		crawlFunc = zhihuCron.BuildZhihuCrawlFunc("", taskID, &zhihuCron.FilterConfig{Include: include, Exclude: exclude, LastCrawl: ""}, &zhihuCron.BaseService{RedisService: h.redisService, CookieService: h.cookieService, Notifier: h.notifier, DB: h.db})
