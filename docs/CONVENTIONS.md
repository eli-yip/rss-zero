...
- **plan** 说清 _怎么做_，**是关键决策的落点**（不另开决策文档），也是「开工前评审」的对象。
  **plan 评审通过后，须经作者（人）手动确认放行方可开工 —— 代理自评不替代此确认；作者可对某次改动明确豁免。**
...
- **每份 plan 两轮独立评审，评审者 ≠ 作者。**（1）_开工前 plan 评审_：对着 issue 重读一遍 ——
  是否完整、决策是否站得住、有没有列出测试与文档更新。**评审后停下等作者（人）手动确认放行，
  未确认不得开工**；代理自评只是喂给这次确认、不替代它（作者可对某次改动明确豁免）。（2）
  _合并前实现评审_：对 diff 跑 `/code-review`（Standards + Spec 两轴并行），结论要么当场修，
  要么开成后续 issue —— 不落单独文件。
