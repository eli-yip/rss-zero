...
来源：2026-07-15 部署 `26.7.8` 时检查生产启动日志发现。`cmd/server/main.go` 的配置初始化成功日志会把
完整 config 结构序列化，包含对象存储、LLM、数据库、通知服务等凭据。应单开安全 issue/plan：日志只保留
非敏感的配置来源或显式白名单字段，并为配置日志增加不泄露 secret 的回归测试；同时轮换已经进入历史日志
与本次部署会话的现有凭据，并按各服务能力清理或缩短日志保留期。
