...
func (h *XiaobotController) Reformat(c echo.Context) error {
	...
	if err != nil {
		...
		return c.JSON(http.StatusInternalServerError, &ApiResp{Message: "failed to init xiaobot parse service"})
	}
	...
	return c.JSON(http.StatusOK, &ApiResp{Message: "start to reformat xiaobot content"})
}
